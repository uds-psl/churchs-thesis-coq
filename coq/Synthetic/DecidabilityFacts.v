Require Export SyntheticComputability.Synthetic.Definitions Lia.
...
Lemma dec_inf_decidable X p :
  (forall x : X, dec (p x)) -> inf_decidable p.
Proof.
  intros d. exists (fun x => if d x then true else false). intros x. destruct (d x); firstorder congruence.
Qed.

Lemma decidable_decidme {X} {p} :
  decidable p -> forall x : X, p x \/ ~ p x.
Proof.
  intros [f H] x. specialize (H x). destruct (f x); firstorder congruence.
Qed.
...
Proof.
  ...
  - intros [[f] % dec_inf_decidable]. now exists f.
Qed.
